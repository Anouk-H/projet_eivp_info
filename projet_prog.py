...
def Graphe(doc, nom, start=fichi['sent_at'][0], end=fichi['sent_at'][fichi.shape[0]-1]):    #nom correspond au nom de la colonne, ne marque que pour le fichier fichi
    ...
    plt.close()
    A=plt.plot(X,Y)
    B=plt.plot(min_max(doc,nom)[0],min_max(doc,nom)[1], color='black', marker=(5,1))
    C=plt.plot(min_max(doc,nom)[2],min_max(doc,nom)[3], color='black', marker=(5,1))
    D=plt.plot([med,med],[0,doc['sent_at'][doc.shape[0]-1]], color='green')                 #trace une ligne horizontale pour la médiane
    E=plt.plot([moy,moy],[0,doc['sent_at'][doc.shape[0]-1]], color='magenta')               #trace une ligne horizontale pour la moyenne
